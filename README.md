...
The specification describes a way to define a _specific data-type_ for JSON elements and also allows for _complex-data types be defined_ as interfaces.
...
per the JSON specification (UNICODE character \u003A), followed by the Colon Character (":") and the data type. 
...
While using characters such as colons, braces and brackets is valid json, the practice makes the object name inaccessible after a JSON.parse() call in Javascript implementations.  This is fortunate as it forces the script to validate the received object.  A very basic validator implementation is included as an example.
...
where **language** refers to a specific language or specification.

There is a strong argument for assuming **TypeScript** as the the default language standard (which itself follows the pascal style)
...
Below is a very simple JSON-ND parser/validator. [demo](.\testJson-nd.html)
```
<html>
<head>
<script>
